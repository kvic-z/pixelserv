...
char* get_stats(const int sta_offset, const int stt_offset) {
    ...
    if (asprintf(&retbuf, (sta_offset) ? sta_fmt : stt_fmt,
        (sta_offset) ? (long)uptimeStr : (long)uptime, loglvl, kcc, kmx, kvg, krq, count, avg, rmx, tav, tmx, slh, slm, sle, slc, slu, nfe, gif, ico, txt, jpg, png, swf, sta + sta_offset, stt + stt_offset, ufe, rdr, nou, pth, noc, opt, pst, hed, bad, tmo, cls, cly, err
        ) < 1)
        retbuf = " <asprintf error>";
