...
char* get_stats(const int sta_offset, const int stt_offset) {
    ...
    if (asprintf(&uptimeStr, "%dd %02d:%02d", (int)uptime/86400, (int)(uptime%86400)/3600, (int)((uptime%86400)%3600)/60) < 1
        ...
        (sta_offset) ? (long)uptimeStr : (long)uptime, log_get_verb(), kcc, kmx, kvg, krq, count, avg, rmx, tav, tmx, slh, slm, sle, slc, slu, v13, v12, v10, uca, ucb, uce, ush, sct, sch, scm, scp, sst, ssh, ssm, ssp, nfe, gif, ico, txt, jpg, png, swf, sta + sta_offset, stt + stt_offset, ufe, opt, pst, hed, rdr, nou, pth, noc, bad, tmo, cls, cly, clt, ers
        ) < 1)
        retbuf = " <asprintf error>";
