...
char* get_stats(const int sta_offset, const int stt_offset) {
  ...
  asprintf(&retbuf, "%.0f uts, %d req"
    ", %d avg, %d rmx, %d err, %d tmo, %d cls, %d nou, %d pth, %d nfe, %d ufe, %d gif, %d bad, %d txt, %d jpg, %d png, %d swf, %d ico, %d ssl, %d sta, %d stt, %d 204, %d rdr"
    , uptime, count, avg, rmx, err, tmo, cls, nou, pth, nfe, ufe, gif, bad, txt, jpg, png, swf, ico, ssl, sta + sta_offset, stt + stt_offset, noc, rdr
  );
