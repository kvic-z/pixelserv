...
char* get_stats(const int sta_offset, const int stt_offset) {
    ...
    if (asprintf(&retbuf, (sta_offset) ? sta_fmt : stt_fmt,
        (sta_offset) ? (long)uptimeStr : (long)uptime, access_log, count, avg, rmx, tav, tmx, slh, slm, sle, slc, slu, nfe, gif, ico, txt, jpg, png, swf, sta + sta_offset, stt + stt_offset, ufe, rdr, nou, pth, noc, pst, hed, opt, bad, tmo, cls, cly, err
        ) < 1)
        retbuf = " <asprintf error>";
